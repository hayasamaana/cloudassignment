...
def conversionRequest(videoId):
    if not videoId in valid_keys():
        message = {
            'status': 400,
            'message': 'Invalid key: ' + videoId,
        }
        resp = jsonify(message)
        resp.status_code = 400
        return resp
    else:
        convertedVideoId = str(uuid.uuid4())+'-'+videoId
        msg = {}
        msg["type"] = "conversionRequest"
        msg["videoId"] = videoId
        msg["convertedVideoId"] = convertedVideoId
        messenger.conversionRequest(msg)
        message = {
                'status': 202,
                'message': 'url for converted video: ' + convertedVideoId,
        }
        resp = jsonify(message)
